...
class _AdminOrdersScreenState extends State<AdminOrdersScreen> {
  ...
  final List<String> _statuses = ['PENDING', 'PROCESSED', 'SHIPPED', 'COMPLETED', 'CANCELLED'];
  ...
  Future<void> _updateStatus(int pesananId, String newStatus) async {
    ...
    if (result['success']) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(result['message'] ?? 'Gagal memperbarui status')),
      );
    }
  }
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Card(
                          ...
                          child: ExpansionTile(
                            leading: Icon(Icons.shopping_bag, color: _getStatusColor(pesanan.status)),
                            title: Text(
                              '${pesanan.kodePesanan} (${pesanan.user?.nama ?? 'N/A'})',
                              style: TextStyle(fontWeight: FontWeight.bold),
                            ),
                            subtitle: Text('Total: ${_formatCurrency(pesanan.totalBayar)}'),
                            trailing: Chip(
                              label: Text(
                                pesanan.status,
  ...
  Widget _buildOrderDetail(Pesanan pesanan) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text('Tanggal Pesan: ${DateFormat('dd MMMM yyyy HH:mm').format(pesanan.tanggalPesanan)}', style: TextStyle(color: Colors.grey[700])),
          SizedBox(height: 8),
          Text('Alamat Kirim: ${pesanan.alamatKirim}, ${pesanan.kotaTujuan}', style: TextStyle(fontWeight: FontWeight.w500)),
          if (pesanan.catatan != null && pesanan.catatan!.isNotEmpty) 
            Padding(
              padding: const EdgeInsets.only(top: 4.0),
              child: Text('Catatan: ${pesanan.catatan}', style: TextStyle(fontStyle: FontStyle.italic)),
            ),
          SizedBox(height: 12),
          Text('Produk Dipesan:', style: TextStyle(fontWeight: FontWeight.bold)),
